...
세 사람의 직업은 승무원·모델·연주자처럼 화려하지만 첫 만남은 현실이 아니라 집에서 하는 온라인 게임이다. 채원은 `막차요정`, 유나는 `무보정`, 소희는 `쉼표`라는 닉네임과 도트 아바타만 사용한다. 플레이어가 집에서 게임을 2·4·6회 반복하면 첫 파티 → 아무것도 증명하지 않는 정기 길드 → 작은 집의 오프라인 식탁이 순서대로 열린다.

오프라인 모임 전에는 현실 이름·직업·초상화를 알 수 없다. 게임에서 개인 연락처를 묻거나 현실 성공을 과시하거나 셋 중 누구와 데이트할지 고르려 하면 세 사람이 차갑게 거절하고 다음 단계가 늦어진다. 파티의 속도를 맞추고 아픈 사람에게 죽을 보내며 소박한 관계를 지켜야 여섯 번째 밤 뒤 본명과 연락처가 공개된다.

윤세라와 동거 중이어도 첫 오프라인 게임 모임에는 따라오지 않는다. 세라는 게임이 자기 취향이 아니라고 말한 뒤, 집 밖을 두려워하던 플레이어가 스스로 약속을 잡고 나가는 것이 다행이라며 현관에서 보내 준다. 모임 상대가 채원·유나·소희라는 사실이 공개됐을 때 위험한 3인조 공동연애 또는 세 사람 중 한 명과 연애 중이면 강유진의 알림이 먼저 도착한다. 귀가 뒤에는 현재 연인인 유진·채린·세라만 등장해 설명을 듣거나 질투를 부정하거나 다시 외출할 경계를 합의한다. 위험 3인조와 자유인 3인조의 실제 대면은 이 첫 모임을 습격하는 방식이 아니라, 며칠 뒤 정식으로 잡은 두 번째 식사에서 이루어진다.
