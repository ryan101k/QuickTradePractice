...
`romance_route_guard.js`의 `romanceRoutes`가 현재 진행 중인 그룹을 하나만 유지한다. 한 그룹의 선택형 장면이 끝나기 전에 다른 그룹의 시작 사건을 예약할 수 없고, 완료·실패한 사건은 다시 처음부터 발생하지 않는다. 다음 그룹이 성립할 때는 기존 `relationshipGroup`과 `polycule.members`를 합쳐 저장하므로 앞 그룹 구성원이 사라지지 않는다.
