...
시장 경쟁자는 랭킹과 차트에는 처음부터 존재하지만 휴대폰 연락처에는 전원 자동 등록되지 않는다. 플레이어가 주식을 보유했거나 3개월 이상 시장에 남아 이름이 알려진 뒤 경쟁자가 먼저 연락할 수 있다. 경쟁자가 공격했을 때, 또는 플레이어가 그 세력을 직접 조사·협상·공격했을 때도 해당 인물만 `contactUnlocked`가 된다. 기존 저장에서 이미 주고받은 메시지가 있다면 연락처를 유지한다.
...
연락처는 연인만이 아니라 시작 배경에서 생긴 부모·친구·직업 인맥과 AI 시장 경쟁자도 포함한다. 경쟁자는 매달 최대 한 명이 실제 종목 등락, 자기 세력의 손익·압박 단계·휴전 제안에 맞춰 먼저 연락하고, 답장은 근거 질문·반론·정보 교환·짧은 응답으로 구성한다. 경쟁자 연락은 연애 연락보다 월말 큐 우선순위가 높고 `rivalFeed`에도 대화 결과를 남긴다. 캐릭터 대사를 추가할 때 범용 `chat_lines.js`만 늘리지 말고 `character_dialogue.js`의 해당 인물 말투를 우선한다.
...
장태식은 AI 세력 수장이지만 플레이어를 직접 공격하는 후보에서는 제외한다. 첫 부하는 연락처의 `첫 부하 · 상황 보고`로 등록되며, 여사친·연인 수, 부채, 스트레스, 최근 공격자에 따라 다른 보고와 잔소리를 보낸다.
