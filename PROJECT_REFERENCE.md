...
직원은 신입·경력·팀장/비공개 경력자별 매출 기여와 월 인건비가 다르며, 최소 등급인 신입도 정상 사기·평판에서 추가 인건비보다 큰 매출을 만든다. 사업 카드에는 직원 매출 기여, 월 인건비, 채용 뒤 최소 예상 순익과 확장 뒤 예상 순익을 함께 표시한다. `expand()`에서 `expansionCost()`를 다시 호출하면 `ensure()` 정규화로 현재 사업 객체 참조가 낡아져 현금만 차감되고 단계가 저장되지 않을 수 있으므로, 확장 비용은 확보한 현재 객체에서 계산한다.
