...
한채린을 알고 있고 책임자 둘 이상이 얼굴을 공개하면 `사람을 사는 방식` 교차 이사회가 열린다. 채린은 네 사람을 개별 스카우트하지만 모두에게 거절당한다. 플레이어는 채린에게 인사권 없는 투자자 자리만 주거나, 사람을 사려는 투자를 완전히 거절할 수 있다.
...
- 재접촉: 최초 비공개 회동 뒤에는 세력 규모를 다시 검사하지 않는다. 개인 연락처가 열릴 때까지 인맥 창에 월 1회 후속 회동이 나타난다.
- 개별 시스템: **복종 만족**
- 핵심 성향: 주인공의 좋은 직업·정상적인 성공보다 자신에게 휘둘리거나 거칠게 대하는 관계에 더 강하게 반응한다.
- 개인 스토리: 8장. `낮춰진 의자 → 후계 경쟁 → 계약서 없는 관계 → 던져진 계약서 → 아무도 없는 회의실 → 값이 없는 부탁 → 왕관 아래의 흉터 → 문이 닫힌 뒤의 명령`
