...
사교 모임은 `공개 네트워킹 → 업계 라운지 → 비공개 포럼 → 대표자 살롱` 순으로 열린다. 참가할 때 사교 실적과 사회 평판이 오르며 상위 모임에서 박지수·한이슬·오혜린·차서윤을 소개받는다. 소개 시 실명과 원소속은 알 수 있지만, 담당 사업에서 3개월 흑자를 내고 업무 신뢰를 쌓아야 개인 연락처가 열린다. 연락처 직후에는 친구 외출만 가능하고 개인 이야기 3장을 마쳐야 연애가 열린다.
...
과거의 4인 합의형 이사회 연애와 공동 엔딩은 신규 진행에서 사용하지 않는다. 기존 공동 컷신 자산은 삭제하지 않고, 추후 네 회사의 입찰전·평판전·인재 쟁탈전 같은 비연애 업계 경쟁 사건으로 재사용한다.
