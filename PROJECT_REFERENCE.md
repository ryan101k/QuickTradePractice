...
시장 변동성은 `app.js`의 `CFG`, `CAP_META`, 가격 틱 함수와 `events_market.js`의 이슈 강도가 함께 결정한다. 대형·중형·소형주의 기본 틱 한도와 세션 한도는 `CAP_META`에 있고, 최종 게임형 보정은 `QT_MARKET_BALANCE`가 담당한다. 양봉은 2.35배·최소 +0.25%로 확대하고 음봉은 0.48배로 완충한다. 상승 한도는 기본의 1.5배, 하락 한도는 0.5배다. 플레이어가 실제 보유한 종목에는 포트폴리오 비중에 따른 우호 수급이 더해지며 공매도는 반대 방향을 사용한다. 뉴스는 단순 장식이 아니라 가격 반영·기업 보고서·장 마감 보고에 연결된다.
...
월말 자유시간은 **행동 종류와 관계없이 최대 4회**다. 동일 행동군도 반복할 수 있으며 `monthActions`에는 불리언 대신 횟수를 누적한다. 구버전의 `true` 값은 1회로 호환한다. 초반 현금이 부족하면 `이번 주 돈 벌기`에서 본업 추가 근무·온라인 단기 의뢰·하루 일당 업무를 선택해 즉시 돈을 받을 수 있다. 수입 행동은 반복 가능하지만 같은 달에는 피로 때문에 보수가 100%→85%→70%→55%로 감소한다. 별도 직무교육은 없으며 취미의 `자기계발`이 행복·매력과 함께 `CAREER.train()`을 호출해 직무 능력도 올린다.
...
월 행동 허브에는 `다른 행동 보기`나 중첩된 `<details>` 메뉴를 두지 않는다. 집·외출 외의 모든 기능은 **생활·건강 / 가족·인맥 / 세력·법정 / 투자 컨설팅 / 경력 / 거주지 / 자산·사업** 독립 창으로 연다. 치료는 생활·건강 창, 대출 상환은 자산·사업 창의 금융 탭에서 처리한다. 집에서 아무 일정 없이 쉬는 비용은 실제 차감과 UI 모두 30,000원이다.

월말 개인 연락은 일반 사건 팝업 대신 휴대전화 잠금화면 알림으로 시작한다. 알림을 누르면 대화방이 올라오고 상대 메시지·입력 중 표시·답장 선택지·후속 답변이 순서대로 재생된다. 월 1회 답장 및 관계 수치 처리 규칙은 기존 연락 엔진을 그대로 사용한다.
...
집 화면은 `pixelHomeHTML()`이 현재 실거주 등급을 도트풍 생활공간으로 만들고, `LUXURY_GOODS`의 게임 환경·시계·스포츠카·미술품·요트를 보유 상태에 따라 공간에 배치한다. 사치품 구매는 월 행동력을 쓰지 않는 성취 소비이며 월 유지비도 없다. 생활 격과 사회 평판은 외출 장소 해금과 화려한 성향의 첫인상 보정에 사용하되, UI에는 세부 계산값 대신 소박함·여유로움 같은 단계 문구를 우선 표시한다.
...
`business.js`가 사업 상태와 계산을 소유하고, `business_romance.js`가 네 담당자의 익명 신원·업무 신뢰·유혹 함정·얼굴 공개·연애 엔딩을 소유한다. 세력은 전투·방어·영향력 조직이고, 사업체는 직원·매출·비용·평판을 관리하는 독립 수익 조직이다.

사업 종류는 담당자마다 3개, 총 12개다.
...
`business.js` 핵심 API는 `start`, `expand`, `close`, `setStrategy`, `assetValue`, `projected`, `monthly`, `eventView`, `resolveEvent`다. 각 사업에는 여성 담당자 한 명이 배정되고 월간 보고와 선택형 문제 사건이 있다. 업종 전용 사건이 없는 경우에도 담당 직군별 공통 사건이 발생하므로, 직원은 장식이 아니라 플레이어에게 연락해 납기·계약·안전·운영 문제의 결정을 요청한다.
...
4. `네 사업장을 도는 하루`: 서로 다른 현장을 함께 확인하는 지점 순회
5. `같이 번 돈의 사용법`: 여덟 사업체와 개인 이야기 3장을 완성한 뒤 공동 성과 배분

공동 선택은 `업무 시너지`, `공동 의사결정`, `공과 사 경계`를 바꾼다. 5장을 완료하고 세 지표와 전원 관계 조건을 충족해야 `네 개의 명함` 세트 엔딩이 열린다. 추가 공동 도트 컷신은 `event-business-quartet-branch-tour-pixel-v1.png`, `event-business-quartet-payday-pixel-v1.png`다.
...
세트의 위험은 감금이 아니라 내부 판정값인 **회귀 압력**이다. 숫자는 플레이어에게 직접 노출하지 않고 ‘복구된 단체방 → 되살아난 옛 규칙 → 과거가 현재를 덮는 중’처럼 관계 분위기로 표시한다. 다섯은 각자 생활표, 건강 기록, 사진, 이동선, 디지털 백업을 가지고 “지금보다 원래의 너를 더 잘 안다”고 주장한다. 단체 사건은 시작 친구 호감 32 → 다섯 전원 친구 → 전원 호감 48·신뢰 35 순서로 자연 발생하며, 외부 연인이 있으면 최종 졸업식은 보류한다.
...
## 16. 미구현·추후 작업 — 남은 히로인 세트

아래 두 조합은 아직 게임 로직에 연결하지 않은 기획안이다. 세트명, 조건, 이벤트 수치와 엔딩 명칭은 추후 개편 시 확정한다. 나래는 투자 상담 역할과 별개로 히로인 자격을 유지한다.

| 후보 | 구성 | 임시 주제 | 이야기 방향 | 도트 컷신 방향 |
|---|---|---|---|---|
| 검증하는 세 사람 | 나래, 혜진, 아린 | 정답과 불확실성 | 투자 원칙·연구 증거·원고 검토로 플레이어를 검증하던 세 사람이 완벽한 답 대신 사람을 받아들이는 관계 | 청록·금색, 야간 차트·연구실·편집실 |
| 돌보는 세 사람 | 하은, 수아, 다은 | 돌봄과 휴식 | 간호·교육·제과로 타인을 돌보던 세 사람에게 플레이어가 일방적으로 기대는지, 서로의 휴식을 지키는지가 갈림길 | 크림·주황·새벽 남색, 병원 휴게실·교실·폐점 후 제과점 |

- 임시 결말 후보는 각각 `미완성으로 남긴 결론 / 모범답안 인생`, `아무도 필요하지 않은 휴일 / 빈 의자 셋`이다.
- 컷신은 개인별 2장, 합동 4장을 임시 기준으로만 기록하며 실제 제작 시 다시 조정한다.
- 기존 개인 스토리 점검이 선행되며, 현재 세트 판정이나 연락·엔딩 조건에는 포함하지 않는다.
