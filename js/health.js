...
const CONDITIONS=[
 {id:'burnout',name:'번아웃',icon:'🫥',minAge:20,stress:65,chance:.10,health:-2,stress:6,cost:1200000},
 {id:'back',name:'허리디스크',icon:'🦴',minAge:25,stress:45,chance:.045,health:-2,stress:2,cost:2500000},
 {id:'hypertension',name:'고혈압',icon:'🫀',minAge:35,stress:55,chance:.045,health:-2,stress:1,cost:1800000},
 {id:'diabetes',name:'당뇨 전단계',icon:'🩸',minAge:40,stress:35,chance:.035,health:-3,stress:1,cost:3000000},
 {id:'heart',name:'심혈관 질환',icon:'❤️‍🩹',minAge:55,stress:60,chance:.025,health:-6,stress:3,cost:12000000},
 {id:'cancer',name:'중증 질환',icon:'🏥',minAge:60,stress:30,chance:.018,health:-8,stress:5,cost:30000000},
];
...
function monthly(life,ctx){
 ensure(life);const news=[];const age=ctx.age||25;
 ...
 for(const c of CONDITIONS){
  if(life.conditions.includes(c.id)||age<c.minAge||life.stress<c.stress)continue;
  const ageMul=1+Math.max(0,age-c.minAge)/35;
  if(Math.random()<c.chance*ageMul){life.conditions.push(c.id);news.push(`${c.icon} ${c.name} 진단`);break;}
 }
 const naturalRisk=age<70?0:Math.pow((age-69)/35,2)*.035;
 const criticalRisk=life.health<20?(20-life.health)*.008:0;
 const died=life.health<=0||Math.random()<naturalRisk+criticalRisk;
...
function treat(life){const c=treatmentOffer(life);if(!c)return null;life.conditions=life.conditions.filter(id=>id!==c.id);life.health=clamp(life.health+12,0,100);life.stress=clamp(life.stress-8,0,100);return c;}
