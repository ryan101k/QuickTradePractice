...
const ARCS={
 ...
 '한채린':['초대장에 없던 상석','계열사 이름이 적힌 계좌','계약서 없는 관계','아첨에는 식고 자기 명령을 거절하는 무례에 빠지는 상속녀가 사적인 굴복, 대등함, 권력 공모 중 하나를 선택한다.'],
};
...
const SPECIAL={
 ...
 '강유진':[
  {title:'동거인 확인서의 빈칸',min:18,scene:'event-yujin-rain-rescue.png',desc:'개인 번호를 교환한 뒤 유진이 첫 방문 조사 기록의 수정본을 가져왔습니다. 윤세라의 거처와 사건 관계는 정리됐지만, 당신과 세라의 관계를 적는 칸만 비어 있습니다. 수사와 무관하다던 질문을 왜 두 번이나 했는지 이제는 업무 밖에서 물을 수 있습니다.',speaker:'그날 애인이냐고 물은 건 신변 보호 때문이었어요. 적어도 처음에는요. 그런데 왜 대답을 듣기가 싫었는지는… 경찰 보고서에 적을 수가 없네요.',choices:[
 ...
 '한채린':[
  {title:'초대장에 없던 상석',min:18,scene:'event-chaerin-contract.png',desc:'사교모임에서 처음 마주친 날처럼 채린이 낮은 의자를 다시 준비했습니다. 다만 이번에는 수행원을 전부 내보내고 자기 상석 하나만 남겨뒀습니다. 모두가 비위를 맞추는 삶에서, 채린은 당신이 또 자기 자리를 빼앗아주기를 기다립니다.',speaker:'이번에는 어느 자리에 앉을 거야? 내 옆이라고 말하면 재미없고, 아래에 앉으면 다시는 안 부를 거야.',choices:[
   C('command','채린의 상석에 앉아 보고할 게 있으면 서서 말하라고 한다','채린이 기다린 무례를 망설임 없이 되풀이한다',15,7,9,'neutral','그건 내 자리야. …그래도 비키라고 하지는 않을게. 네가 언제까지 그렇게 말할 수 있는지 보고 싶어.',{},'command'),
   C('equal','낮은 의자를 치우고 같은 높이의 의자 두 개를 직접 놓는다','굴복도 지배도 아닌 자리를 행동으로 요구한다',11,14,-6,'good','다들 내 자리를 원하거나 내 아래에 앉았어. 같은 높이를 가져온 사람은 네가 처음이네.',{},'equal'),
   C('conspire','상석을 양보하는 대신 채린의 투자와 경호를 요구한다','비위를 맞추지 않지만 첫 관계부터 권력 거래로 묶는다',7,-4,13,'bad','적어도 영광이라고는 안 하네. 좋아, 내 자리는 빌려줄게. 대신 네 자리도 내가 살 거야.',{cash:5000000,morality:-4},'conspire')
  ]},
  {title:'계열사 이름이 적힌 계좌',min:32,scene:'event-chaerin-thrown-contract.png',desc:'강유진이 추적한 차명계좌는 채린의 계열 경호사에서 끊겼고, 윤세라가 숨긴 원본 장부에는 그 회사가 피해자의 주소와 동선을 사들인 기록이 남아 있습니다. 채린은 본사 승인 없는 일이라고 잘라 말하지만 유진은 책임을, 세라는 원본을 들고 물러서지 않습니다.',speaker:'경찰은 절차가 필요하고 세라는 복수가 필요하겠지. 나는 계열사를 오늘 안에 잘라낼 돈이 있어. 너는 셋 중 누구 비위를 맞출 건데?',choices:[
   C('command','“변명 말고 네 이름으로 공개하고 피해자부터 갚아”라고 명령한다','채린의 돈과 이름으로 계열사 범죄를 직접 수습하게 한다',15,13,8,'neutral','내 이름을 명령문에 넣는 사람도 처음이네. 좋아. 오늘은 네가 시킨 순서대로 무릎 꿇려줄게.',{morality:6},'command'),
   C('equal','유진에게 원장을 넘기고 세라와 피해자 보상안을 함께 만든다','수사·증거·자금을 어느 한 사람의 소유로 만들지 않는다',11,16,-7,'good','내 돈만 받거나 경찰한테 전부 넘길 줄 알았는데, 셋 다 일하게 만드네. 마음에 안 들 만큼 공평해.',{cash:-1000000,morality:7},'equal'),
   C('conspire','계열사를 살려두고 장부를 경쟁 세력 역공에 사용한다','범죄의 통로를 두 사람만의 정보망으로 바꾼다',10,-5,17,'bad','유진은 싫어하겠고 세라는 좋아하겠네. 나는 네가 착한 척 안 해서 마음에 들어.',{cash:9000000,morality:-12,guilt:6},'conspire')
  ]},
  {title:'계약서 없는 관계',min:46,scene:'event-chaerin-private-dinner.png',desc:'채린이 결혼과 재산, 비밀유지 조항이 담긴 계약서를 가져왔지만 서명란은 비워뒀습니다. 명령하면 무엇이든 주겠다는 사람이 정작 당신에게는 거절당하고 싶어 한다는 모순을 처음 인정합니다.',speaker:'돈으로 묶으면 다들 남았어. 그런데 너까지 남으면 내가 좋아한 사람이 사라지는 기분이야. 찢을 거면 내 앞에서 찢어.',choices:[
   C('command','계약서를 접어 돌려주고 다시는 돈으로 관계를 설명하지 말라고 한다','채린이 원한 거절을 책임 있는 명령으로 바꾼다',16,12,8,'neutral','또 명령하네. 싫다고 해야 하는데… 네가 나한테 원하는 게 돈이 아니라는 말처럼 들려.',{},'command'),
   C('equal','각자의 재산과 자유를 보장하는 공동 합의서를 새로 쓴다','보호 장치는 두되 소유가 아닌 동반 관계를 택한다',13,17,-8,'good','처음으로 계약서가 족쇄가 아니라 서로를 지키는 문서처럼 보여.',{},'equal'),
   C('conspire','내게 더 유리한 조건과 계열사 의결권을 요구한다','채린의 방식으로 채린을 이기며 권력 관계를 굳힌다',9,-5,15,'bad','그래, 값을 부르면서도 내 눈치는 안 보네. 아주 비싼 공범을 샀다고 생각할게.',{cash:12000000,morality:-6},'conspire')
  ]}
