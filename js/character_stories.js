...
const ARCS={
 ...
 '강유진':['혼자 다녀온 병원','내부 비리 제보','반납한 비상 출입권','모든 위기에서 가장 먼저 불리고 싶은 마음이 경찰 권한과 통제가 되지 않도록 선을 배운다.'],
 '윤세라':['열두 통을 지운 밤','어깨 하나만큼의 거리','두 개의 열쇠','서로의 폐허를 알아본 두 사람이 열린 문으로 구원받을지, 같은 문을 안에서 잠글지 선택한다.'],
 '한채린':['남겨 둔 종잇조각','화가 나야 하는데','왕관을 내려놓는 방','신입을 장난감처럼 시험하던 상속녀가 뺨의 충격 뒤에 깨어난 피학 충동을 끝까지 부정하다가, 합의된 사적 욕망과 공개적 폭력을 구분하는 법을 배운다.'],
};
...
const SPECIAL={
 ...
 '강유진':[
  {title:'혼자 다녀온 병원',min:18,scene:'event-yujin-secret-hospital.webp',desc:'첫 경찰조사에서 은둔 생활과 무너진 식사 기록을 확인한 유진은 당신을 병원에 데려갔습니다. 며칠 뒤 당신이 혼자 재진을 받고 돌아오자 유진은 장바구니와 구급함을 든 채 현관에서 처방 봉투를 발견합니다. 무사해서 다행이라는 말보다 자신을 부르지 않은 이유를 먼저 묻고, 그 서운함이 사랑이 아니라 필요한 사람이 되고 싶은 욕망에서 왔음을 알아챕니다.',speaker:'검사 결과가 괜찮아서 다행이에요. 그런데 왜 혼자 갔는지가 더 화나요. 아픈 게 아니라, 나를 필요로 했던 순간을 놓친 것 같아서 이러는 거라는 것도 알아요.',choices:[
   C('depend','다음부터 몸에 이상이 생기면 가장 먼저 유진을 부르겠다고 한다','유진에게 모든 위기의 첫 번째 자리를 약속한다',14,9,12,'neutral','약속한 거예요. 병원도 경찰도 아버님도 아니고, 다음에는 저부터 불러요.',{},'depend'),
   C('boundary','도움이 필요할 때는 연락하되 혼자 진료받을 권리도 존중해 달라고 한다','돌봄을 받으면서도 호출 순위를 사랑의 증거로 만들지 않는다',11,15,-8,'good','알겠어요. 연락이 없다는 걸 버림받았다는 뜻으로 해석하지 않을게요. 어려워도 먼저 물어볼게요.',{},'boundary'),
   C('complicity','유진을 단독 비상 연락처로 등록하고 모든 진료 기록을 공유한다','업무용 보호 권한을 둘만의 독점적인 연결로 바꾼다',8,-3,16,'bad','그러면 다음에는 놓치지 않겠네요. 병원보다 먼저 알고, 누구보다 먼저 갈게요.',{morality:-4},'complicity')
  ]},
  ...
  {title:'응답 없는 마흔일곱 분',min:88,scene:'event-yujin-emergency-entry.webp',desc:'당신이 정기 검진을 취소한 뒤 연락 없이 잠들자 유진은 마흔일곱 분 동안 전화를 걸었습니다. 마지막에는 경찰 비상 조회와 관리실 출입 권한을 사용해 현관문을 열었습니다. 소파에서 무사히 자는 당신을 발견한 순간 다리가 풀릴 만큼 안도했지만, 실제 위험보다 자신이 가장 먼저 찾아내야 한다는 확신으로 선을 넘었다는 사실도 부정하지 못합니다.',speaker:'살아 있어서 다행이에요. 그런데 내가 열지 않았으면 어쩔 뻔했냐는 생각이 먼저 들었어요. 아무 일도 없었는데, 내가 필요했다는 결론부터 만들고 들어온 거예요.',choices:[
   C('depend','다음에도 연락이 끊기면 망설이지 말고 들어오라고 한다','유진의 과잉 구조를 가장 확실한 사랑의 증거로 받아들인다',17,12,17,'neutral','그 말을 허락으로 기억할게요. 다음에도 제일 먼저 찾아내고, 문을 열고, 당신이 괜찮을 때까지 안 나갈게요.',{},'depend'),
   C('boundary','출입 기록을 지우지 말고 반납한 뒤 실제 위험 기준을 함께 정하자고 한다','위반을 숨기지 않게 하고 구조가 필요한 조건을 구체적으로 합의한다',14,21,-12,'good','기록도 권한도 반납할게요. 다음에는 제 공포가 아니라 우리가 정한 기준을 먼저 확인하겠습니다.',{},'boundary'),
   C('complicity','현관 비밀번호와 단독 비상연락 권한을 정식으로 넘긴다','경찰 권한으로 연 문을 영구적인 독점 보호 계약으로 바꾼다',12,-7,24,'bad','이제 허가 없이 들어온 게 아니게 되네요. 당신이 연락을 받지 않는 순간부터는 전부 제가 판단할게요.',{morality:-12,guilt:6},'complicity')
  ]},
 ...
 '한채린':[
  ...
  {title:'화가 나야 하는데',min:48,scene:'event-chaerin-4.png',desc:'뺨을 맞은 뒤 채린은 며칠 동안 연락하지 않았습니다. 다시 만난 채린은 멍이 옅어지는 것을 매일 확인했다고 고백하면서도 아픈 것이 좋았던 건 절대 아니라고 먼저 부정합니다. 하지만 자신이 먼저 모욕하고 반응을 끌어냈으며, 충격보다 당신이 다시 오지 않을 가능성을 더 무서워했다는 사실까지는 숨기지 못합니다.',speaker:'아픈 걸 좋아하는 게 아니야. 그런 취향 아니라고. 그런데 멍이 사라지는 걸 보니까 네가 정말 떠날 것 같아서… 그게 더 싫었어.',choices:[
  ...
  {title:'명령을 부탁하는 여자',min:56,scene:'event-chaerin-private-command.png',desc:'채린은 당신의 약속을 멋대로 취소하고 휴대폰을 빼앗은 뒤, 화를 내지 않자 오히려 초조해집니다. “앉아, 따라와, 연락 끊어”라고 명령을 늘어놓지만 그 모든 말은 당신이 자기 말을 끊고 가혹하게 꾸짖어 주기를 기다리는 도발입니다. 본인은 피학 욕망이 아니라 단지 주도권을 시험하는 것뿐이라고 우깁니다.',speaker:'왜 가만히 있어? 내가 네 일정을 지웠고 연락도 막았어. 화내야 정상 아니야? …오해하지 마. 맞고 싶다는 게 아니라, 네가 어디까지 할 수 있는지 보는 거야.',choices:[
  ...
  {title:'화내지 않는 사람',min:64,scene:'event-chaerin-private-command.png',desc:'채린은 다시 강한 반응을 얻으려고 집 앞에 사람을 세우고 약속 시간을 바꾸고 세라와 유진에게까지 자기 명령을 전달합니다. 하지만 당신은 손도 목소리도 높이지 않고 모든 지시를 취소합니다. 공개된 자리에서 상대를 몰아붙여 폭력을 끌어내는 것은 취향도 합의도 아니며, 채린이 정말 원하는 가혹함은 둘만의 공간에서 언제든 멈출 수 있게 부탁해야 한다고 말합니다.',speaker:'차라리 화를 내. 내가 잘못했다고 말하면 되잖아. …밖에서 이러는 건 안 된다는 것도 알아. 그런데 직접 해달라고 부탁하면 내가 정말 그런 사람이 되는 것 같단 말이야.',choices:[
  ...
  {title:'빌려 입힌 재킷',min:88,scene:'event-chaerin-9.png',desc:'가족 만찬에서 채린은 완벽한 후계자 얼굴을 유지했지만 차에 타자마자 재킷을 당신에게 다시 걸쳐줍니다. 사랑받으려면 더 유능하거나 더 복종해야 했던 어린 시절과, 공개적으로 약해질 수 없었던 이유를 처음 끝까지 말합니다. 그리고 자신이 원한 것은 패배가 아니라 가혹하게 다뤄진 뒤에도 버려지지 않는 확신이었다고 인정합니다.',speaker:'내가 지는 걸 좋아한 게 아니야. 아프게 혼나고도 네가 남아 있는 걸 확인하고 싶었던 거야. …말로 하니까 정말 이상하네. 그래도 이제는 부정만 하진 않을게.',choices:[
