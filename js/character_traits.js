...
const SYSTEMS={
 ...
 '한채린':{key:'submission',icon:'👑',name:'굴복 욕구',scene:'event-chaerin-contract.png',monthly:()=>-1,actions:{경력:0,라이벌:2,인맥:1}},
 ...
 '채원':{key:'returnHome',icon:'✈️',name:'돌아올 곳',good:true,scene:'event-chaewon-7.png',monthly:(l,c)=>c.relationshipStable?4:-2,actions:{휴식:5,가족:5,경력:-1}},
 '유나':{key:'scandal',icon:'📸',name:'스캔들 열기',scene:'event-yuna-2.png',monthly:(l,c)=>c.lovers*7+(c.reputation>45?3:-1),actions:{데이트:7,인맥:4,휴식:-3}},
 ...
 '소희':{key:'freedom',icon:'🎤',name:'자유의 여백',good:true,scene:'sohee-evnet-3.png',monthly:(l,c)=>c.relationshipStable?2:-1,actions:{취미:8,데이트:4,가족:-2}},
...
function monthly(life,ctx){const out=[],c=context(life,ctx),active=new Set(['friend','casual','partner','lover','polycule']);(life.met||[]).forEach(rec=>{const s=system(rec.name);if(!s||s.external||!active.has(rec.status))return;const result=change(rec,s.monthly(life,c));if(result&&result.changed)out.push({rec,...result});});return out;}
function stageText(rec){const s=system(rec.name),st=ensure(rec),n=stageOf(s,st.value);if(rec.name==='강유진')return['원칙적 거리','걱정','구원 강박','공범적 보호'][n];if(rec.name==='한채린')return['무관심','거슬리는 관심','기대하는 무례','사적인 굴복'][n];if(rec.name==='유나')return['조용함','소문','스캔들','대중 폭발'][n];return s.good?['낯섦','관심','유대','고유 보너스'][n]:['안정','징후','갈등','위험'][n];}
root.QT_CHARACTER_TRAITS={SYSTEMS,system,ensure,label,stageOf,stageText,context,change,monthly};
})(window);
