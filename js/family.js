...
function ensure(life){
 ...
 if(!Number.isFinite(life.familyBond))life.familyBond=35;if(!life.playerName)life.playerName='나';
 life.children.forEach(child=>{
  if(!Array.isArray(child.caregivers)||!child.caregivers.length){
   const current=root.QT_RELATIONSHIPS?root.QT_RELATIONSHIPS.caregiverNames(life):[];
   child.caregivers=[life.playerName,child.otherParent,...current].filter(Boolean).filter((name,index,list)=>list.indexOf(name)===index);
  }
 });
 return life;
}
...
function caregiverNames(life,extra){ensure(life);const current=root.QT_RELATIONSHIPS?root.QT_RELATIONSHIPS.caregiverNames(life):[];return[life.playerName,...current,...(extra||[])].filter(Boolean).filter((name,index,list)=>list.indexOf(name)===index);}
function startPlan(life,method,meta){ensure(life);if(life.familyPlan)return{ok:false,message:'이미 가족 계획이 진행 중입니다.'};if(life.children.length>=4)return{ok:false,message:'현재는 자녀 4명까지 가능합니다.'};
 const adoption=method==='adopt',m=meta||{},caregivers=Array.isArray(m.caregivers)&&m.caregivers.length?caregiverNames(life,m.caregivers):caregiverNames(life,m.otherParent?[m.otherParent]:[]);
 life.familyPlan={method:adoption?'입양':'출산',months:adoption?6:9,cost:adoption?12000000:5000000,origin:m.origin||'family_agreement',otherParent:m.otherParent||null,caregivers,secret:!!m.secret};return{ok:true,plan:life.familyPlan};}
function createChild(life){const plan=life.familyPlan||{},trait=pick(TRAITS),available=NAMES.filter(n=>!life.children.some(c=>c.name===n)),name=pick(available.length?available:NAMES);const c={id:'child-'+Date.now()+'-'+Math.random(),name,ageMonths:0,trait:trait.id,talent:trait.talent,education:0,bond:50,happy:70,health:90,origin:plan.origin||'family_agreement',otherParent:plan.otherParent||null,caregivers:Array.isArray(plan.caregivers)&&plan.caregivers.length?plan.caregivers.slice():caregiverNames(life),secret:!!plan.secret};life.children.push(c);if(life.dangerousTrioBond&&life.dangerousTrioBond.active)life.familyRouteLock='dangerous';return c;}
function monthly(life){ensure(life);const news=[];let cost=0,birth=null;
 ...
 if(life.parentAge<100){life.parentAge+=1/12;const decay=life.parentAge>70?.8:.25;life.parentHealth=clamp(life.parentHealth-decay,0,100);if(life.parentHealth<35&&Math.random()<.12)news.push('👨 아버지의 건강이 좋지 않아 돌봄이 필요합니다.');}
...
function caregiversOf(child){return Array.isArray(child&&child.caregivers)?child.caregivers:[];}
function caregiverLabel(child){const names=caregiversOf(child);return names.length?names.join('·'):'주 양육자 기록 없음';}
function syncCaregivers(life,names){ensure(life);const additions=[life.playerName,...(names||[])].filter(Boolean);life.children.forEach(child=>{child.caregivers=[...(child.caregivers||[]),...additions].filter((name,index,list)=>list.indexOf(name)===index);});return life.children;}
root.QT_FAMILY={NAMES,TRAITS,ensure,childAge,stage,monthlyCost,startPlan,monthly,educate,bond,careParents,heirs,bestHeir,traitOf,caregiverNames,caregiversOf,caregiverLabel,syncCaregivers};
})(window);
