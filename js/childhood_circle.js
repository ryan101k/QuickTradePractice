/* QuickTrade Life — 소꿉친구 5인조 ‘졸업하지 못한 다섯’
 * 오래 안다는 사실이 애정이면서 동시에 소유권처럼 변하는 관계다.
 * 윤세라 일행의 감금과 달리, 현재의 주인공을 학창 시절 모습으로 되돌리는
 * ‘회귀 압력’이 위험도 역할을 한다.
 */
(function (root) {
  'use strict';
  ...
  const META = {
    예린: { role:'생활표', icon:'🗓️', danger:'가족 일정과 생활 습관을 근거로 지금의 하루까지 대신 정한다.' },
    보라: { role:'건강기록', icon:'💊', danger:'어릴 때 몸 상태를 기억한다는 이유로 식사와 치료 결정권을 가져간다.' },
    서연: { role:'기억보관', icon:'📷', danger:'사진과 그림 속 과거의 모습이 진짜 당신이라며 현재의 변화를 지운다.' },
    나영: { role:'도주차단', icon:'🏃‍♀️', danger:'말없이 사라지면 익숙한 동선부터 뒤져 직접 찾아 데려온다.' },
    미래: { role:'기록복원', icon:'💾', danger:'삭제한 채팅과 계정을 복구해 변화를 임시 오류처럼 취급한다.' },
  };
  ...
  const LINES = {
    예린: {
      first:['졸업하고도 네 소식은 다 들었어. 네가 말 안 한 것까지. 반갑다고 해도 되지?'],
    ...
    보라: {
      first:['얼굴색부터 봤어. 반가운 건 그다음이고. 여전히 아프면 숨기지?'],
    ...
    서연: {
      first:['사람은 변했는데 웃기 직전 표정은 그대로네. 그 얼굴, 나만 알아봤어.'],
    ...
    나영: {
      first:['도망치는 걸음걸이 그대로네. 반갑다, 야. 이번에도 내가 먼저 잡았지?'],
    ...
    미래: {
      first:['계정 바꿔도 문장 끝에 점 두 개 찍는 건 같네.. 접속 확인, 소꿉친구 복구 완료.'],
  ...
  const EVENTS = {
    reunion: {
      ...
      speakers:[
        ...
        ['보라','요즘 네 몸 상태를 모르는 사람은 있어도, 예전 걸 모르는 사람은 여기 없어.'],
        ['서연','새로 만난 사람들은 완성된 얼굴만 보잖아. 우리는 밑그림부터 봤고.'],
        ['나영','또 도망치면 내가 잡는다. 농담 반이야.'],
