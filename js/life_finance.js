...
function ensure(life){if(!life.finance)life.finance={policies:[],taxesPaid:0,premiumsPaid:0,claims:0,unemploymentMonths:0};if(!Array.isArray(life.finance.policies))life.finance.policies=[];['taxesPaid','premiumsPaid','claims','unemploymentMonths'].forEach(key=>{if(!Number.isFinite(Number(life.finance[key])))life.finance[key]=0;});return life.finance;}
...
function monthly(life,ctx){const f=ensure(life),plans=active(life);const premiums=plans.reduce((s,p)=>s+p.premium,0);const tax=incomeTax(ctx.income||0);const propertyTax=Math.round(Math.max(0,ctx.propertyValue||0)*.004/12);let incomeBenefit=0;
 ...
 f.taxesPaid+=tax+propertyTax;f.premiumsPaid+=premiums;return{premiums,tax,propertyTax,incomeBenefit,net:incomeBenefit-premiums-tax-propertyTax};}
...
root.QT_LIFE_FINANCE={POLICIES,ensure,subscribe,cancel,active,monthly,treatmentCost,deathBenefit,removeLegacyPension,incomeTax};
})(window);
