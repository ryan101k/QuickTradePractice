...
const LIFE_EVENTS = [
  {
    ...
    options: [
      { text: '상자와 세라를 함께 데려온다', effects: { cash:-120000, happy:5, morality:5, meetSera:true }, outcome: '세라는 차에 탄 뒤에도 목적지를 말하지 않았습니다. 당신이 “갈 데 없으면 일단 와”라고 하자 처음으로 휴대전화를 켰습니다. 저장된 자료에는 그 세력의 송금책과 동선이 남아 있었습니다.' },
      { text: '빈 작업실을 정리하고 임시 숙소를 잡아준다', effects: { cash:-50000, happy:3, morality:3, meetSera:true }, outcome: '세라는 열쇠보다 먼저 당신 번호를 저장했습니다. “빚은 갚을게요. 그 사람들 어디로 돈 빼는지도 알아요.” 감사와 집착을 구분하기 어려운 첫 연락이 도착했습니다.' },
      { text: '경찰과 피해자 지원 연락처만 남기고 나온다', effects: { happy:-2 }, outcome: '세라는 종이를 받아 들었지만 전화하지 않았습니다. 다음에 찾아왔을 때 작업실은 비어 있었습니다.' },
    ],
  },
  /* ---------------- 직업(job) ---------------- */
  {
...
/* 빚이 커지면 장태식이 먼저 찾아온다 — 큰돈을 급하게 융통하는 위험한 지름길 */
LIFE_EVENTS.push(
  {
    id:'taesik_bigloan', cat:'debt', emoji:'🦈', title:'장태식이 찾아왔다',
    desc:'빚이 불어난 걸 어떻게 알았는지, 장태식이 명함을 내밀었다. "은행이 더 안 빌려주지? 나는 한도가 없어. 대신… 알지?"',
    cond:c=>!c.makjang && (c.loan||0)>=40000000,
    options:[
      {text:'급한 대로 크게 빌린다 (현금 +3천만 · 사채 +6천만)', effects:{cash:30000000, debt:60000000, happy:-10}, outcome:'당장 숨통은 트였지만 갚아야 할 돈이 두 배로 불었다. 이자가 무섭다.'},
      {text:'조금만 빌린다 (현금 +1천만 · 사채 +1.8천만)', effects:{cash:10000000, debt:18000000, happy:-4}, outcome:'최소한만 받았다. 태식은 "또 필요하면 연락해"라며 웃었다.'},
      {text:'정중히 거절한다', effects:{happy:4}, outcome:'"현명하네. 근데 사람 일 모르는 거야." 명함을 두고 사라졌다.'}
    ]
  }
);
